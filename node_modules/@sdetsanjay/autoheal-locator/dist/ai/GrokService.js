...
/**
 * Grok (xAI) AI Service implementation
 */
class GrokService {
    constructor(apiKey, model = 'grok-beta', timeout = 30000) {
        this.apiKey = apiKey;
        this.model = model;
    ...
    async analyzeDOM(html, description, originalSelector, framework = AutomationFramework_1.AutomationFramework.SELENIUM) {
        ...
        try {
            ...
            const tokensUsed = response.data.usage?.total_tokens || 1500;
            return this.parseAIResponse(text, framework, tokensUsed);
        }
        catch (error) {
            console.error('Grok DOM analysis failed:', error.response?.data || error.message);
            throw new Error(`AI DOM analysis failed: ${error.message}`);
        }
    }
    async analyzeVisual(screenshot, description) {
        ...
        try {
            ...
            const text = response.data.choices[0].message.content;
            const tokensUsed = response.data.usage?.total_tokens || 1500;
            return this.parseAIResponse(text, AutomationFramework_1.AutomationFramework.SELENIUM, tokensUsed);
        }
        catch (error) {
            console.error('Grok visual analysis failed:', error.response?.data || error.message);
            throw new Error(`AI visual analysis failed: ${error.message}`);
        }
    }
    async selectBestMatchingElement(elements, _description) {
        if (elements.length === 1) {
            return elements[0];
        }
        // For simplicity, return the first element
        // In production, you'd use AI to disambiguate
        console.warn(`Multiple elements found (${elements.length}), returning first. Consider using AI disambiguation.`);
        return elements[0];
    }
    getProviderName() {
        return 'Grok (xAI)';
    }
    buildDOMPrompt(html, description, originalSelector, framework) {
        const truncatedHtml = html.length > 15000 ? html.substring(0, 15000) + '...' : html;
        if (framework === AutomationFramework_1.AutomationFramework.PLAYWRIGHT) {
...
Respond with ONLY valid JSON:`;
    ...
    parseAIResponse(text, _framework = AutomationFramework_1.AutomationFramework.SELENIUM, tokensUsed = 1500) {
        try {
...
//# sourceMappingURL=GrokService.js.map