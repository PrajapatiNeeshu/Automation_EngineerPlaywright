...
class GeminiAIService {
    constructor(apiKey, model = 'gemini-2.0-flash-exp', timeout = 30000) {
        this.apiKey = apiKey;
        this.model = model;
    ...
    async analyzeDOM(html, description, originalSelector, framework = AutomationFramework_1.AutomationFramework.SELENIUM) {
        ...
        try {
            const response = await this.makeRequestWithRetry(`/models/${this.model}:generateContent?key=${this.apiKey}`, {
                contents: [
                    {
                        parts: [{ text: prompt }],
                    },
     ...
     */
    async makeRequestWithRetry(url, data, maxRetries = 3) {
        let lastError;
...
Respond with ONLY valid JSON:`;
    ...
    parseAIResponse(text, _framework = AutomationFramework_1.AutomationFramework.SELENIUM, tokensUsed = 0) {
        try {
