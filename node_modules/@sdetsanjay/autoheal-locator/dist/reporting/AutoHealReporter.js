...
exports.AutoHealReporter = void 0;
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
const LocatorStrategy_1 = require("../models/LocatorStrategy");
/**
...
class AutoHealReporter {
     ...
     */
    getApiEndpointForProvider(provider) {
     ...
     */
    recordSelectorUsage(originalSelector, description, strategy, executionTimeMs, success, actualSelector, elementDetails, reasoning, tokensUsed = 0) {
        const report = {
            originalSelector,
            description,
            strategy,
            ...
            maxTokens: strategy === LocatorStrategy_1.LocatorStrategy.DOM_ANALYSIS ? this.domMaxTokens : this.visualMaxTokens,
            temperature: strategy === LocatorStrategy_1.LocatorStrategy.DOM_ANALYSIS ? this.domTemperature : this.visualTemperature,
            retryCount: this.maxRetries,
            promptType: strategy === LocatorStrategy_1.LocatorStrategy.DOM_ANALYSIS ? 'DOM Analysis' : 'Visual Analysis',
            promptTokens: 0,
            completionTokens: 0,
        };
    ...
    logToConsole(report) {
        ...
        const tokenInfo = report.tokensUsed > 0 &&
            (report.strategy === LocatorStrategy_1.LocatorStrategy.DOM_ANALYSIS || report.strategy === LocatorStrategy_1.LocatorStrategy.VISUAL_ANALYSIS)
            ? ` [${report.tokensUsed} tokens]`
            : '';
        console.log(`[${status}] [${strategyName}] [${report.executionTimeMs}ms]${tokenInfo} ${report.originalSelector} → ${report.success ? report.actualSelector : 'FAILED'}`);
        if (report.originalSelector !== report.actualSelector && report.success) {
            console.log(`   [HEALED] ${report.reasoning}`);
        }
    }
    getStrategyShortName(strategy) {
        switch (strategy) {
            case LocatorStrategy_1.LocatorStrategy.ORIGINAL_SELECTOR:
                return 'ORIGINAL';
            case LocatorStrategy_1.LocatorStrategy.DOM_ANALYSIS:
                return 'DOM-AI';
            case LocatorStrategy_1.LocatorStrategy.VISUAL_ANALYSIS:
                return 'VISUAL-AI';
            case LocatorStrategy_1.LocatorStrategy.CACHED:
                return 'CACHE';
            default:
                return 'UNKNOWN';
        }
     ...
     */
    generateHTMLReport(outputDir = './autoheal-reports') {
     ...
     */
    generateJSONReport(outputDir = './autoheal-reports') {
        this.ensureDirectoryExists(outputDir);
        const filename = path.join(outputDir, `${this.testRunId}_Report.json`);
        const stats = this.calculateStatistics();
        const hasAI = this.reports.some(r => r.strategy === LocatorStrategy_1.LocatorStrategy.DOM_ANALYSIS || r.strategy === LocatorStrategy_1.LocatorStrategy.VISUAL_ANALYSIS);
        const jsonReport = {
            reportMetadata: {
            ...
            healingActivities: this.reports.map(r => ({
                ...
                aiImplementation: (r.strategy === LocatorStrategy_1.LocatorStrategy.DOM_ANALYSIS || r.strategy === LocatorStrategy_1.LocatorStrategy.VISUAL_ANALYSIS) ? {
     ...
     */
    generateTextReport(outputDir = './autoheal-reports') {
        this.ensureDirectoryExists(outputDir);
    ...
    printSummary() {
        ...
        console.log(`Total: ${this.reports.length} | Success: ${stats.successful} | Failed: ${this.reports.length - stats.successful}`);
        console.log(`Original: ${stats.originalStrategy} | DOM Healed: ${stats.domHealed} | Visual: ${stats.visualHealed} | Cached: ${stats.cached}`);
        if (stats.totalTokens > 0) {
            console.log(`Token Usage - Total: ${stats.totalTokens} | DOM: ${stats.domTokens} | Visual: ${stats.visualTokens}`);
...
<body>
    ...
    generateAISectionHTML(stats) {
        const aiReport = this.reports.find(r => r.strategy === LocatorStrategy_1.LocatorStrategy.DOM_ANALYSIS || r.strategy === LocatorStrategy_1.LocatorStrategy.VISUAL_ANALYSIS);
        if (!aiReport)
            return '';
        return `
...
${report.tokensUsed > 0 && (report.strategy === LocatorStrategy_1.LocatorStrategy.DOM_ANALYSIS || report.strategy === LocatorStrategy_1.LocatorStrategy.VISUAL_ANALYSIS) ? `   Tokens: ${report.tokensUsed}\n` : ''}${report.success ? `   Actual Selector: ${report.actualSelector}
...
`;
    ...
    calculateStatistics() {
        ...
        const originalStrategy = this.reports.filter(r => r.strategy === LocatorStrategy_1.LocatorStrategy.ORIGINAL_SELECTOR).length;
        const domHealed = this.reports.filter(r => r.strategy === LocatorStrategy_1.LocatorStrategy.DOM_ANALYSIS).length;
        const visualHealed = this.reports.filter(r => r.strategy === LocatorStrategy_1.LocatorStrategy.VISUAL_ANALYSIS).length;
        const cached = this.reports.filter(r => r.strategy === LocatorStrategy_1.LocatorStrategy.CACHED).length;
        const totalTokens = this.reports.reduce((sum, r) => sum + r.tokensUsed, 0);
        const domTokens = this.reports
            .filter(r => r.strategy === LocatorStrategy_1.LocatorStrategy.DOM_ANALYSIS)
            .reduce((sum, r) => sum + r.tokensUsed, 0);
        const visualTokens = this.reports
            .filter(r => r.strategy === LocatorStrategy_1.LocatorStrategy.VISUAL_ANALYSIS)
            .reduce((sum, r) => sum + r.tokensUsed, 0);
        return {
            successful,
            originalStrategy,
...
exports.AutoHealReporter = AutoHealReporter;
//# sourceMappingURL=AutoHealReporter.js.map